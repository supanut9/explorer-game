# Documentation Index

Start here when contributing to this repository.

## Start Here
- [AGENTS.md](../AGENTS.md)
- [docs/game-spec.md](game-spec.md)
- [docs/repo-standards.md](repo-standards.md)
- [docs/ai-workflow.md](ai-workflow.md)
- [docs/decision-rules.md](decision-rules.md)

## Rules
- [docs/architecture.md](architecture.md)
- [docs/content-pipeline.md](content-pipeline.md)
- [docs/content-decisions.md](content-decisions.md)
- [docs/content-plan.md](content-plan.md)

## Planning
- [docs/planning/README.md](planning/README.md)
- [docs/planning/backlog.md](planning/backlog.md)
- [docs/planning/sprint-01.md](planning/sprint-01.md)
- [docs/planning/sprint-02.md](planning/sprint-02.md)
- [docs/planning/sprint-03.md](planning/sprint-03.md)
- [docs/planning/sprint-04.md](planning/sprint-04.md)
- [docs/planning/sprint-05.md](planning/sprint-05.md)
- [docs/planning/sprint-06.md](planning/sprint-06.md)
- [docs/planning/sprint-07.md](planning/sprint-07.md)
- [docs/planning/sprint-08.md](planning/sprint-08.md)
- [docs/planning/sprint-09.md](planning/sprint-09.md)

## Skills
- [docs/task-skills.md](task-skills.md)
- [docs/agent-roles.md](agent-roles.md)
- [.agents/skills/backlog-refinement/SKILL.md](../.agents/skills/backlog-refinement/SKILL.md)
- [.agents/skills/sprint-planning/SKILL.md](../.agents/skills/sprint-planning/SKILL.md)
- [.agents/skills/unity-bootstrap/SKILL.md](../.agents/skills/unity-bootstrap/SKILL.md)

## Codex
- [.codex/config.toml](../.codex/config.toml)
- [.codex/agents/content-designer.toml](../.codex/agents/content-designer.toml)
- [.codex/agents/game-designer.toml](../.codex/agents/game-designer.toml)
- [.codex/agents/runtime-implementer.toml](../.codex/agents/runtime-implementer.toml)
